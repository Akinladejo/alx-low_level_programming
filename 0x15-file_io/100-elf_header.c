...
int main(int argc, char **argv)
{
	...
	read_bytes = read(elf, e_ident, EI_NIDENT);
	...
	if (e_ident[EI_DATA] == ELFDATA2MSB)
			read_bytes = read(elf, &e_ident[EI_PAD], 8);

	print_type(*(unsigned int *)&e_ident[EI_PAD], e_ident);
	print_entry(*(unsigned long int *)&e_ident[EI_PAD + 4], e_ident);
